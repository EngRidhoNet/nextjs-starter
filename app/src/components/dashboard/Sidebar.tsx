...
import { 
  LogOut, X, LayoutDashboard, Bot, BarChart3, Users, Settings,
  Search, HelpCircle, Bell, Sparkles, ChevronRight
} from "lucide-react";

const NAV_ITEMS = [
  { icon: LayoutDashboard, label: "Dashboard", href: "/dashboard" },
  { icon: Bot, label: "Sarah AI", href: "/dashboard/ai", badge: "AI" },
  { icon: BarChart3, label: "Marketing", href: "/dashboard/marketing" },
  { icon: Users, label: "Team", href: "/dashboard/team", count: null },
...
export function Sidebar({ isMobile, isOpen, onClose }: SidebarProps) {
      ...
      <div className="px-5 pt-5 pb-3">
        <div className="flex items-center justify-between mb-5">
          <div className="flex items-center gap-2.5">
            ...
            <Link href="/dashboard" className="font-bold text-[17px] text-gray-900 tracking-tight">
              Winsta
            </Link>
        ...
        <div className="relative">
          ...
          <input
            type="text"
            placeholder="Search..."
            className="w-full h-9 pl-9 pr-12 text-sm bg-gray-50 border border-gray-200/80 rounded-lg placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-rose-500/20 focus:border-rose-300 transition-all"
          />
          <kbd className="absolute right-2.5 top-1/2 -translate-y-1/2 text-[10px] text-gray-400 bg-white border border-gray-200 rounded px-1.5 py-0.5 font-medium">
            ⌘F
          </kbd>
        </div>
      </div>
        ...
        {NAV_ITEMS.map((item) => {
          ...
          return (
            <Link
              ...
              className={[
                "group w-full flex items-center gap-3 px-3 py-2 rounded-lg text-[13px] font-medium transition-all duration-150",
                isActive
      ...
      <div className="mt-auto border-t border-gray-100">
        ...
        <div className="px-3 py-2 space-y-0.5">
          ...
          <button className="w-full flex items-center gap-3 px-3 py-2 rounded-lg text-[13px] font-medium text-gray-600 hover:bg-gray-50 hover:text-gray-900 transition-colors relative">
            ...
            <span className="ml-auto flex items-center justify-center h-5 w-5 rounded-full bg-rose-500 text-white text-[10px] font-bold animate-subtle-pulse">
              3
            </span>
          </button>
        </div>
        ...
        <div className="px-3 py-3 border-t border-gray-100">
          <div className="flex items-center gap-3 px-2 py-2 rounded-lg hover:bg-gray-50 transition-colors cursor-pointer group">
            <Avatar className="h-9 w-9 ring-2 ring-gray-100 group-hover:ring-rose-100 transition-all">
          ...
          <button
            onClick={logout}
            className="mt-1 w-full flex items-center justify-center gap-2 px-3 py-2 text-[13px] text-gray-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-colors"
          >
  ...
  return (
    <>
      {isMobile && isOpen && <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-40" onClick={onClose} />}
      {sidebar}
    </>
  );
}
